...
`);
  ...
  var segmentMgr = new segment_saver(audioSegmentIndex, videoSegmentIndex, key, keyId,
    decryptScript, playlistPath, downloadPath, mergePath, endPlayTime,
    conf.max_segment_num, audioPLName, videoPLName, lastSegmentURI,
    dashParser.manifestExpired);

  dashParser.manifestExpired = false;

  lastSegmentURI = await segmentMgr.processSegments(audioMediaPLTemplate, videoMediaPLTemplate,
    updateDuration);
  ...
  if (currentTimeInSecond > dashParser.expireTime_) {
    ...
    dashParser.reset();
    await dashParser.getManifestURI_();
  ...
  updateDuration = await dashParser.requestManifest_();
}
